...
unsigned char fuzz_buf[1024000];
  ...
  #define IJON_SET(x) ((void)0)
  #define IJON_MAX_AT(id, x) ((void)0)
#else
...
int main(int argc, char** argv)
{
    ...
    while (__AFL_LOOP(std::numeric_limits<unsigned int>::max())) {  // increase if you have good stability
        ...
        unsigned int prev_state = dut.state;

        dut.reset_n = 1;
        for (ssize_t i = 0; i < len; i++) {
            dut.code = buf[i];
            tick(dut);
            unsigned int curr_state = dut.state;

            IJON_SET((prev_state << 8) | curr_state); // reward unique transitions
